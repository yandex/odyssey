...
void
mm_coroutine_cache_init(mm_coroutine_cache_t *cache,
                        int stack_size,
...
void
mm_coroutine_cache_free(mm_coroutine_cache_t *cache)
{
	mm_list_t *i, *n;
...
void
mm_coroutine_cache_stat(mm_coroutine_cache_t *cache,
                        uint64_t *count,
                        uint64_t *count_free)
{
...
mm_coroutine_cache_pop(mm_coroutine_cache_t *cache)
{
	...
	coroutine =
	  mm_coroutine_allocate(cache->stack_size, cache->stack_size_guard);
	if (coroutine == NULL)
		cache->count_total--;
	return coroutine;
}
...
void
mm_coroutine_cache_push(mm_coroutine_cache_t *cache, mm_coroutine_t *coroutine)
{
