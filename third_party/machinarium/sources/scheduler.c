...
static void mm_scheduler_main(void *arg)
{
	...
	/* wakeup joiners */
	machine_list_t *i;
	machine_list_foreach(&coroutine->joiners, i)
	{
		mm_coroutine_t *joiner;
		joiner = machine_container_of(i, mm_coroutine_t, link_join);
...
{
	machine_list_init(&scheduler->list_ready);
	machine_list_init(&scheduler->list_active);
	scheduler->id_seq = 0;
...
void mm_scheduler_free(mm_scheduler_t *scheduler)
{
	...
	machine_list_t *i, *p;
	machine_list_foreach_safe(&scheduler->list_ready, i, p)
	{
		coroutine = machine_container_of(i, mm_coroutine_t, link);
		mm_coroutine_free(coroutine);
	}
	machine_list_foreach_safe(&scheduler->list_active, i, p)
	{
		coroutine = machine_container_of(i, mm_coroutine_t, link);
		mm_coroutine_free(coroutine);
	}
...
void mm_scheduler_run(mm_scheduler_t *scheduler, mm_coroutine_cache_t *cache)
{
	while (scheduler->count_ready > 0) {
		mm_coroutine_t *coroutine;
		coroutine = machine_container_of(scheduler->list_ready.next,
						 mm_coroutine_t, link);
...
mm_coroutine_t *mm_scheduler_find(mm_scheduler_t *scheduler, uint64_t id)
{
	mm_coroutine_t *coroutine;
	machine_list_t *i;
	machine_list_foreach(&scheduler->list_ready, i)
	{
		coroutine = machine_container_of(i, mm_coroutine_t, link);
		if (coroutine->id == id)
			return coroutine;
	}
	machine_list_foreach(&scheduler->list_active, i)
	{
		coroutine = machine_container_of(i, mm_coroutine_t, link);
		if (coroutine->id == id)
			return coroutine;
		      ...
		      mm_coroutinestate_t state)
{
	...
	machine_list_unlink(&coroutine->link);
	machine_list_init(&coroutine->link);
	if (target != NULL)
		machine_list_append(target, &coroutine->link);
...
{
	machine_list_append(&coroutine->joiners, &joiner->link_join);
}
