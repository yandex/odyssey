...
void mm_channel_init(mm_channel_t *channel)
{
	...
	machine_list_init(&channel->msg_list);
	channel->msg_list_count = 0;

	machine_list_init(&channel->readers);
...
void mm_channel_free(mm_channel_t *channel)
{
	if (channel->msg_list_count == 0)
		return;
	machine_list_t *i, *n;
	machine_list_foreach_safe(&channel->msg_list, i, n)
	{
...
mm_retcode_t mm_channel_write(mm_channel_t *channel, mm_msg_t *msg)
{
	...
	if (channel->readers_count) {
		...
		reader = machine_container_of(channel->readers.next,
					      mm_channelrd_t, link);
		reader->result = msg;
	...
	machine_list_append(&channel->msg_list, &msg->link);
...
mm_msg_t *mm_channel_read(mm_channel_t *channel, uint32_t time_ms)
{
	...
	if ((channel->msg_list_count > 0) && (channel->readers_count == 0)) {
		next = machine_list_pop(&channel->msg_list);
		channel->msg_list_count--;
		mm_sleeplock_unlock(&channel->lock);
		return machine_container_of(next, mm_msg_t, link);
	}
	...
	machine_list_append(&channel->readers, &reader.link);
	channel->readers_count++;
...
mm_msg_t *mm_channel_read_back(mm_channel_t *channel, uint32_t time_ms)
{
	...
	machine_list_t *next;
	if ((channel->msg_list_count > 0) && (channel->readers_count == 0)) {
		next = machine_list_pop_back(&channel->msg_list);
		channel->msg_list_count--;
		mm_sleeplock_unlock(&channel->lock);
		return machine_container_of(next, mm_msg_t, link);
	}
