				  ...
				  const char *field_name)
{
	...
	if (value < (int64_t)INT_MIN || value > (int64_t)INT_MAX) {
		od_cfg_diag_error(diags, location,
				  "value '%s' = %ld is out of integer bounds",
				  field_name, value);
		return -1;
	}
...
static int od_cfg_validate_db(od_cfg_database_t *db, od_cfg_diag_list_t *diags)
{
...
{
	for (size_t i = 0; i < model->databases_count; ++i) {
		int rc = od_cfg_validate_db(model->databases[i], diags);
		if (rc != 0) {
			return rc;
		}
	}
