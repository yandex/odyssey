...
static int convert_rule_settings(const od_cfg_route_t *cfg, od_list_t *spools, od_rules_t *rules, od_rule_t *rule, od_cfg_diag_list_t *diags)
{
	...
	if (cfg->shared_pool.seen.is_set && cfg->pool_size.seen.is_set) {
		od_cfg_diag_error(diags, cfg->location, "shared_pool and pool_size can't be set simultaneously");
		goto error;
	}
...
static int convert_shared_pool(convert_ctx_t *ctx, const od_cfg_shared_pool_t *cfg)
{
			 ...
			 od_hba_rules_t *hba_rules, od_cfg_diag_list_t *diags)
{
	...
	if (regcomp(&ctx.rfc952_hostname_regex,
		    ...
		    REG_EXTENDED)) {
		od_cfg_diag_error(diags, model->location, "can't compile hostname regex");
		return -1;
	}
