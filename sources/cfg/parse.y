...
/*
 * machinarium stacks are small, so need to use heap and
 * limit initial size
 */
%code top {
...
%code {
	/* bison 3.x generates yynerrs in pure parsers but doesn't use it */
...
%token <str> STRING
%token <num> NUMBER
%token <num> SIZE_NUMBER

%destructor { od_free($$); } <str>
%destructor { od_free($$.address); } <selector>
...
group:
	GROUP string_value route_selector
		{
...
user_named:
	USER string_value route_selector
		{
			od_cfg_route_t *user = od_cfg_database_add_user(ctx->current_database,
																 $2,
																 0 /* is_default */,
...
/*
 * route_selector — optional address + optional conn_type in any order.
 *
 * Supported combinations (matching old parser behaviour):
 *   <empty>
 *   "addr"
 *   conn_type
 *   "addr" conn_type
...
route_selector:
	  %empty
		{ $$.address = NULL; $$.conn_type = OD_CFG_CONN_TYPE_DEFAULT; }
	| string_value
		{ $$.address = $1; $1 = NULL; $$.conn_type = OD_CFG_CONN_TYPE_DEFAULT; }
	| conn_type
		{ $$.address = NULL; $$.conn_type = $1; }
	| string_value conn_type
		{ $$.address = $1; $1 = NULL; $$.conn_type = $2; }
	| conn_type string_value
		{ $$.address = $2; $2 = NULL; $$.conn_type = $1; }
	;
	...
	| SERVER_LIFETIME int_value
		{
			if ($2 < 0) {
				od_cfg_diag_error(ctx->diags, @1,
								  "server_lifetime must be non-negative");
			} else {
				od_cfg_route_t *ur = ctx->current_user;
