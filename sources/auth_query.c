...
static inline int
od_auth_query_do(od_server_t *server,
                 char *query,
                 int len,
                 kiwi_password_t *result)
{
	...
	od_debug(
	  &instance->logger, "auth_query", server->client, server, "%s", query);
...
od_auth_query_format(od_rule_t *rule,
                     kiwi_var_t *user,
                     char *peer,
                     char *output,
                     int output_len)
	...
	while (format_pos < format_end) {
		if (*format_pos == '%') {
			...
			switch (*format_pos) {
				case 'u':
					len = od_snprintf(
					  dst_pos, dst_end - dst_pos, "%s", user->value);
					dst_pos += len;
					break;
				case 'h':
              ...
              kiwi_password_t *password)
{
	...
	kiwi_var_set(&auth_client->startup.user,
	             KIWI_VAR_UNDEF,
	             rule->auth_query_user,
	             strlen(rule->auth_query_user) + 1);

	kiwi_var_set(&auth_client->startup.database,
	             KIWI_VAR_UNDEF,
	             rule->auth_query_db,
