...
int od_reset(od_server_t *server)
{
	...
	if (route->rule->pool->reset_timeout_ms > 0 && route->rule->pool->reset_timeout_ms < UINT32_MAX) {
		reset_timeout_ms = (uint32_t) route->rule->pool->reset_timeout_ms;
	}
	...
	if (!od_server_synchronized(server)) {
		...
		od_frontend_status_t st = od_service_stream_server_until_rfq("reset", server, reset_timeout_ms);
		if (st != OD_OK) {
			od_log(&instance->logger, "reset", server->client, server,
				"rfq waiting failed - closing, status = %d (%s)", st, od_frontend_status_to_str(st));
			goto drop;
		}
	...
	if (route->rule->pool->rollback) {
		if (server->is_transaction) {
			...
			rc = od_backend_query(
				...
				sizeof(query_rlb), reset_timeout_ms);
			if (rc == NOT_OK_RESPONSE) {
				goto error;
			}
