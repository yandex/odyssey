...
void
od_backend_error(od_server_t *server, char *context, char *data, uint32_t size)
{
	...
	while (1) {
		...
		switch (type) {
			case KIWI_BE_READY_FOR_QUERY:
				od_backend_ready(
				  server, machine_msg_data(msg), machine_msg_size(msg));
			...
			case KIWI_BE_BACKEND_KEY_DATA:
				rc = kiwi_fe_read_key(
				  machine_msg_data(msg), machine_msg_size(msg), &server->key);
				machine_msg_free(msg);
				if (rc == -1) {
					od_error(&instance->logger,
					         "startup",
			...
			case KIWI_BE_PARAMETER_STATUS: {
				...
				if (route_params) {
					kiwi_param_t *param;
					param =
					  kiwi_param_allocate(name, name_len, value, value_len);
					if (param)
                      ...
                      od_rule_storage_t *storage)
{
	...
	if (rc == -1) {
		od_error(
		  &instance->logger, context, NULL, server, "failed to set server io");
		machine_close(io);
		machine_io_free(io);
		return -1;
	}
	...
	if (storage->host) {
		...
		if (strchr(storage->host, ':')) {
			...
			rc_resolve =
			  inet_pton(AF_INET6, storage->host, &saddr_v6.sin6_addr);
...
int
od_backend_update_parameter(od_server_t *server,
                            char *context,
                            char *data,
                            uint32_t size,
                            int server_only)
...
int
od_backend_ready_wait(od_server_t *server,
                      char *context,
                      int count,
                      uint32_t time_ms)
{
	...
	for (;;) {
		...
		if (type == KIWI_BE_PARAMETER_STATUS) {
			...
			int rc;
			rc = od_backend_update_parameter(
			  server, context, machine_msg_data(msg), machine_msg_size(msg), 1);
			if (rc == -1) {
				machine_msg_free(msg);
				return -1;
			}
			...
			od_backend_error(
			  server, context, machine_msg_data(msg), machine_msg_size(msg));
			machine_msg_free(msg);
			continue;
...
int
od_backend_query(od_server_t *server,
                 char *context,
                 char *query,
                 int len,
                 uint32_t timeout)
