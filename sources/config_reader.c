...
od_config_reader_open(od_config_reader_t *reader, char *config_file)
{
	...
	return 0;
error:
	od_errorf(reader->error, "failed to open config file '%s'", config_file);
	return -1;
}
...
static void
od_config_reader_error(od_config_reader_t *reader,
                       od_token_t *token,
                       char *fmt,
                       ...)
{
...
static bool
od_config_reader_quantiles(od_config_reader_t *reader,
                           char *value,
                           double **quantiles,
                           int *count)
{
		...
		switch (rc) {
			...
			case OD_PARSER_EOF:
				od_config_reader_error(
				  reader, &token, "unexpected end of config file");
				return -1;
	...
	for (;;) {
		...
		if (keyword == NULL) {
			...
			od_list_foreach(&module->link, i)
			{
				...
				rc =
				  curr_module->config_init_cb(rule->user_name, reader, &token);
