...
static inline int od_console_show_pools_add_cb(od_route_t *route, void **argv)
{
	...
	/* sv_active */
	data_len = od_snprintf(data, sizeof(data), "%d",
			       od_multi_pool_count_active(mpool));
	rc = kiwi_be_write_data_row_add(stream, offset, data, data_len);
	if (rc == NOT_OK_RESPONSE) {
		goto error;
	}
	/* sv_idle */
	data_len = od_snprintf(data, sizeof(data), "%d",
			       od_multi_pool_count_idle(mpool));
	rc = kiwi_be_write_data_row_add(stream, offset, data, data_len);
	if (rc == NOT_OK_RESPONSE) {
		goto error;
	}
...
static inline int od_console_show_servers_cb(od_route_t *route, void **argv)
{
	...
	od_multi_pool_foreach(mpool, OD_SERVER_ACTIVE,
...
static inline int od_console_show_lists_cb(od_route_t *route, void **argv)
{
	...
	od_multi_pool_t *mpool = od_route_get_multi_pool(route);
	(*used_servers) += od_multi_pool_count_active(mpool);
	(*free_servers) += od_multi_pool_count_idle(mpool);
