...
#define IS_LCPRV1(c) \
	((unsigned char)(c) == LCPRV1_A || (unsigned char)(c) == LCPRV1_B)
...
#define IS_LCPRV2(c) \
	((unsigned char)(c) == LCPRV2_A || (unsigned char)(c) == LCPRV2_B)
...
#define CHECK_ENCODING_CONVERSION_ARGS(srcencoding, destencoding)              \
	check_encoding_conversion_args(PG_GETARG_INT32(0), PG_GETARG_INT32(1), \
				       PG_GETARG_INT32(4), (srcencoding),      \
				       (destencoding))

/*
 * Some handy functions for Unicode-specific tests.
 */
static inline bool is_valid_unicode_codepoint(char32_t c)
{
	return (c > 0 && c <= 0x10FFFF);
}

static inline bool is_utf16_surrogate_first(char32_t c)
{
	return (c >= 0xD800 && c <= 0xDBFF);
}

static inline bool is_utf16_surrogate_second(char32_t c)
{
	return (c >= 0xDC00 && c <= 0xDFFF);
}

static inline char32_t surrogate_pair_to_codepoint(char16_t first,
						   char16_t second)
{
 ...
 */
static inline char32_t utf8_to_unicode(const unsigned char *c)
{
	if ((*c & 0x80) == 0) {
		return (char32_t)c[0];
	} else if ((*c & 0xe0) == 0xc0) {
		return (char32_t)(((c[0] & 0x1f) << 6) | (c[1] & 0x3f));
	} else if ((*c & 0xf0) == 0xe0) {
		return (char32_t)(((c[0] & 0x0f) << 12) | ((c[1] & 0x3f) << 6) |
				  (c[2] & 0x3f));
	} else if ((*c & 0xf8) == 0xf0) {
		return (char32_t)(((c[0] & 0x07) << 18) |
				  ((c[1] & 0x3f) << 12) | ((c[2] & 0x3f) << 6) |
				  (c[3] & 0x3f));
	} else {
 ...
 */
static inline unsigned char *unicode_to_utf8(char32_t c,
 ...
 */
static inline int unicode_utf8len(char32_t c)
{
	if (c <= 0x7F) {
		return 1;
	} else if (c <= 0x7FF) {
...
extern int pg_encoding_mb2wchar_with_len(int encoding, const char *from,
					 pg_wchar *to, int len);
extern int pg_wchar2mb(const pg_wchar *from, char *to);
extern int pg_wchar2mb_with_len(const pg_wchar *from, char *to, int len);
extern int pg_encoding_wchar2mb_with_len(int encoding, const pg_wchar *from,
					 char *to, int len);
extern int pg_char_and_wchar_strcmp(const char *s1, const pg_wchar *s2);
extern int pg_wchar_strncmp(const pg_wchar *s1, const pg_wchar *s2, size_t n);
extern int pg_char_and_wchar_strncmp(const char *s1, const pg_wchar *s2,
				     size_t n);
...
extern int pg_encoding_mbcliplen(int encoding, const char *mbstr, int len,
				 int limit);
...
extern int UtfToLocal(const unsigned char *utf, int len, unsigned char *iso,
		      const pg_mb_radix_tree *map,
		      const pg_utf_to_local_combined *cmap, int cmapsize,
		      utf_local_conversion_func conv_func, int encoding,
		      bool noError);
extern int LocalToUtf(const unsigned char *iso, int len, unsigned char *utf,
		      const pg_mb_radix_tree *map,
...
extern void check_encoding_conversion_args(int src_encoding, int dest_encoding,
					   int len, int expected_src_encoding,
					   int expected_dest_encoding);

pg_noreturn extern void report_invalid_encoding(int encoding, const char *mbstr,
						int len);
pg_noreturn extern void report_untranslatable_char(int src_encoding,
						   int dest_encoding,
...
extern int latin2mic(const unsigned char *l, unsigned char *p, int len, int lc,
