...
int
od_auth_frontend_validate_addr(od_config_hba_t *rule, struct sockaddr_in *sin)
{
    ...
    return client_net ^ rule_addr->sin_addr.s_addr;
}

int
od_auth_frontend_validate_addr6(od_config_hba_t *rule, struct sockaddr_in6 *sin)
{
...
int
od_auth_frontend_process_hba_rule(od_client_t *client, od_config_hba_t *rule)
{
    struct sockaddr_storage sa;
    int salen = sizeof(sa);
    int rc    = machine_getpeername(client->io.io, (struct sockaddr *)&sa, &salen);
	if (rc == -1)
		return -1;
	if (sa.ss_family == AF_UNIX) {
		...
		if (rule->connection_type != OD_CONFIG_HBA_HOST || rule->addr.ss_family != AF_INET)
			return -1;
		struct sockaddr_in sin = *(struct sockaddr_in *)&sa;
		rc = od_auth_frontend_validate_addr(rule, &sin);
		if (rc != 0)
			return -1;
	} else if (sa.ss_family == AF_INET6) {
		if (rule->connection_type != OD_CONFIG_HBA_HOST || rule->addr.ss_family != AF_INET6)
			return -1;
		struct sockaddr_in6 sin = *(struct sockaddr_in6 *)&sa;
		rc = od_auth_frontend_validate_addr6(rule, &sin);
		if (rc != 0)
			return -1;
	}

    if (strcmp(client->rule->db_name, rule->database) != 0)
        return -1;
    if (strcmp(client->rule->user_name, rule->user) != 0)
        return -1;
...
od_auth_frontend_hba(od_client_t *client)
{
	...
	od_list_foreach(&instance->config.hba, i)
    {
		...
		rc = od_auth_frontend_process_hba_rule(client, hba_rule);
		if (rc == 0)
			return 0;
	}
