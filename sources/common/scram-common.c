 ...
 * error details.
 */
int scram_SaltedPassword(const char *password, pg_cryptohash_type hash_type,
			 int key_length, const uint8 *salt, int saltlen,
			 int iterations, uint8 *result, const char **errstr)
{
 ...
 * pointing to a message about the error details.
 */
int scram_ClientKey(const uint8 *salted_password, pg_cryptohash_type hash_type,
		    int key_length, uint8 *result, const char **errstr)
{
	...
	if (pg_hmac_init(ctx, salted_password, key_length) < 0 ||
	    pg_hmac_update(ctx, (uint8 *)"Client Key", strlen("Client Key")) <
		    0 ||
 ...
 * pointing to a message about the error details.
 */
int scram_ServerKey(const uint8 *salted_password, pg_cryptohash_type hash_type,
		    int key_length, uint8 *result, const char **errstr)
{
	pg_hmac_ctx *ctx = pg_hmac_create(hash_type);
	...
	if (pg_hmac_init(ctx, salted_password, key_length) < 0 ||
	    pg_hmac_update(ctx, (uint8 *)"Server Key", strlen("Server Key")) <
		    0 ||
	    pg_hmac_final(ctx, result, key_length) < 0) {
		*errstr = pg_hmac_error(ctx);
		pg_hmac_free(ctx);
			 ...
			 const char *password, const char **errstr)
{
	...
	if (scram_SaltedPassword(password, hash_type, key_length, salt, saltlen,
	    ...
	    scram_ClientKey(salted_password, hash_type, key_length, stored_key,
			    errstr) < 0 ||
	    scram_H(stored_key, hash_type, key_length, stored_key, errstr) <
		    0 ||
	    scram_ServerKey(salted_password, hash_type, key_length, server_key,
			    errstr) < 0) {
	...
	maxlen = strlen("SCRAM-SHA-256") + 1 + 10 + 1 /* iteration count */
	...
	*(p++) = '$';
	...
	/* stored key */
	encoded_result =
		pg_b64_encode(stored_key, key_length, p, encoded_stored_len);
	if (encoded_result < 0) {
	...
	*(p++) = ':';
	...
	/* server key */
	encoded_result =
		pg_b64_encode(server_key, key_length, p, encoded_server_len);
	if (encoded_result < 0) {
