...
od_get_execution_lock(char *prefix)
{
	...
	if (prefix == NULL) {
		sprintf(od_exec_lock_name,
		        "%s/%s:%d",
		        prefix,
		        ODYSSEY_LOCK_PREFIX,
		        ODYSSEY_EXEC_LOCK_HASH);
	} else {
		sprintf(od_exec_lock_name,
		        "%s/%s:%d",
	...
	if (fd == -1) {
		od_dbg_printf_on_dvl_lvl(
		  1, "failed to get control lock file due error: %d", errno);
	}
...
od_file_lock_t
od_get_control_lock(char *prefix)
{
	char od_control_lock_name[ODYSSEY_LOCK_MAXPATH];
	if (prefix == NULL) {
		sprintf(od_control_lock_name,
		        "%s/%s:%d",
		        prefix,
		        ODYSSEY_LOCK_PREFIX,
	...
	int fd = open(od_control_lock_name, O_RDWR | O_CREAT, S_IRWXU | S_IRWXG);
