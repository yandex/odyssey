				      ...
				      char *plain_password)
{
	...
	uint8_t salt[SCRAM_DEFAULT_SALT_LEN];
...
static char *calculate_server_signature(od_scram_state_t *scram_state)
{
	...
	int base64_signature_len =
		od_b64_encode(server_signature, OD_SCRAM_MAX_KEY_LEN,
...
#define SCRAM_FINAL_MAX_SIZE 512
	...
	if (sizeof(attributes) - 1 + server_nonce_size >= SCRAM_FINAL_MAX_SIZE) {
		return NULL;
	}
	...
	size += od_b64_encode(client_proof, OD_SCRAM_MAX_KEY_LEN, result + size,
...
od_retcode_t od_scram_verify_server_signature(od_scram_state_t *scram_state,
					      ...
					      size_t auth_data_size)
{
	uint8_t server_signature[SHA256_DIGEST_LENGTH];
				       ...
				       uint8_t **proof_ptr)
{
	...
	if (od_scram_need_channel_binding_check(channel_binding,
		...
		b64_message_len = od_b64_encode((const uint8_t *)cbind_input,
						cbind_input_len, b64_message,
						b64_message_len);
