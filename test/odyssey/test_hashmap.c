...
static inline void test_hashmap_init_item(od_hashmap_elt_t* item,
    void* data, size_t len, od_hash_t* keyhash)
{
    item->data = data;
    item->len = len;
    if (keyhash != NULL) *keyhash = od_murmur_hash(item->data, item->len);
}

// Add element to hashmap by key
void test_hashmap_add_element(od_hashmap_t *hm,
...
void test_hashmap_compare_element_by_key(
    od_hashmap_t *hm,
    void* key_data, size_t key_len, 
    void* value_data, size_t value_len)
{   
...
void test_hashmap_many_items(const size_t hm_size, const size_t element_size){
    ...
    for (size_t i = 0; i < hm_size; ++i) {
        ...
        test_hashmap_add_element(hm, &keys[i], sizeof(i), strings[i], element_size);
    }

    for (size_t i = 0; i < hm_size; ++i) {
        test_hashmap_compare_element_by_key(hm, &keys[i], sizeof(i), strings[i], element_size);
    }
