    ...
    time_t *notafter)
{
	...
	if ((after = X509_get_notAfter(ctx->ssl_peer_cert)) == NULL)
		goto err;
	if (compat_ASN1_time_parse(before->data, before->length, &before_tm, 0) == -1)
		goto err;
	if (compat_ASN1_time_parse(after->data, after->length, &after_tm, 0) == -1)
		goto err;
